...
| **Mobile** pays no extra movement for difficult terrain or hostile figures | **verified** | Consolidated Rules p.47: "Figures that have the Mobile keyword ignore additional movement point costs when entering difficult terrain and spaces containing hostile figures." | `MoveOptions.IgnoresTerrain`, `Pathfinder.EnterCost` |
| **Mobile** may enter and end on blocking and impassable terrain | **verified** | Consolidated Rules p.47: "They ignore terrain for movement, so can move through and enter impassable and blocking terrain, and can end movement in a space containing impassable or blocking terrain." | `Pathfinder.CanOccupy`, `Pathfinder.CanEndOn` |
| A **Mobile** figure on blocking terrain can still be seen, counted to and attacked | **verified** | Consolidated Rules p.47: "If a figure with Mobile occupies a space containing blocking terrain, line of sight can be traced to that figure, spaces can be counted to that figure, and adjacent figures can attack that figure." | `FigureVisibility.CountableOnBlockingTerrain` |
| **Mobile** does NOT gain Massive's figure permissions: it may not end on another figure, and figures still block line of sight to it | **verified** | Absence is the source. The p.47 entry grants terrain permissions only; "can end its movement in spaces that contain ... other figures" and "Figures do not block line of sight to or from a Massive figure" are both said of Massive alone (p.41) | `Pathfinder.CanEndOn`, `Visibility.BlockersFor` |
| **Mobile** does not cross blocking or impassable terrain EDGES | **interpretation** | p.41 gives Massive "can also move through and end movement on blocked or impassable terrain edges"; the Mobile entry names spaces and never edges. Read narrowly on purpose: withholding it can only make a Mobile figure take a longer legal route, whereas granting it could produce a move that is not legal at all | `Pathfinder.StepAllowed` (tests `Massive` only) |
| **Mobile** may not enter a pit | **interpretation** | The p.47 entry names impassable and blocking terrain only, and "pit" appears nowhere in the Consolidated Rules. Treating the general "ignore terrain" clause as covering pits would invent a permission | `Pathfinder.CanOccupy` |
| **Reach** melee attacks may target up to 2 spaces away, in line of sight | **verified** | Consolidated Rules p.51: "A figure with the Reach keyword may perform [melee] attacks that target figures or objects up to 2 spaces away. The target of the attack must be within 2 spaces and in line of sight." Corroborated at p.41: "A [melee] attack with the Reach keyword can target figures or objects in line of sight and within 2 spaces." | `AttackEvaluator.Assess(hasReach)` |
| A **Reach** attack requires no Accuracy | **verified** | Consolidated Rules p.51: "An attack with Reach does not require Accuracy." | `AttackAssessment.RequiredAccuracy` |
| **Reach** adds a way to attack rather than replacing the ordinary adjacent melee | **interpretation** | p.41 lists adjacency and the Reach extension as separate bullets, and states no line-of-sight requirement on the adjacent case. Denying a Reach figure its ordinary adjacent attack would invent a restriction and make the AI decline attacks it may legally make | `AttackEvaluator.Assess` checks adjacency first |
| A figure's speed, attack type, footprint and keywords come from its deployment card | **verified** | `enemies.json` is FFG's own card data. Not a rules question but the defect that made the rules moot: every figure reached the planner as a ranged `Small1x1` at speed 4, so 17 melee cards were planned as ranged and 12 large figures as 1x1 | `UnitProfile.From`, `TrackerBridge.Snapshot`, `SagaBoardController.DeployGroup` |
...
| A group deploys at the active point nearest the Rebels | **house rule** | Not a game rule. Which point a mission means is often set by its own rules text, which the app cannot read, so this is a suggestion the players correct by dragging. It replaces taking whichever point came first in the list. A point with no counted route sorts last but is still offered, since it may be the only one the mission allows | `DeploymentPlanner.RankPoints` |
| On arrival, a square with line of sight to a Rebel is preferred | **house rule** | Not a game rule. A figure deployed into a blind corner spends its first activation walking out of it. Applied only between squares at the same remove from the point, so it never scatters the group | `DeploymentPlanner.PlaceGroup` |
